...
export class MyRRFAddComponent implements OnActivate {
  ...
  submitForm() {
    if (+this.currentRaiseRRFStatus === +RaiseRRFStatus.newRRF) {
      ...
      if (this.validate('rrf', this.newRRF.AdditionalRoles)
        && this.validate('rrf', this.newRRF.PreferredSkills)
        && this.validate('rrf', this.newRRF.QualCertRquired)
        && this.validate('CheckExp', minexp)) {
        this.raiseRRF();
      }

    } else if (+this.currentRaiseRRFStatus === +RaiseRRFStatus.updateRRF) {
      var minexp = this.newRRF.MinExp.toString();
      if (this.validate('rrf', this.newRRF.PreferredSkills)
        && this.validate('rrf', this.newRRF.QualCertRquired)
        && this.validate('CheckExp', minexp)) {
        this.onUpdateClick();
      }
  ...
  getList() {
    for (var i = 0; i < this.interviewRound.length; i++) {
      if (parseInt(this.IntwRound.toString()) === this.interviewRound[i].Id) {
        this.IntRoundValue = this.interviewRound[i].Value;
      }
    }
    for (var j = 0; j < this.practices.length; j++) {
      var abc: number = this.newRRF.Practice.Id;
      if (parseInt(abc.toString()) === this.practices[j].Id) {
