...
import {RRFGridRowNewUIComponent} from '../../shared/components/RRFGridRow/RRFGridNewUI.component';
...
@Component({
    ...
    templateUrl: 'RRFDashboardNewUI.component.html',
    directives: [PanelsAvailablityComponent, ROUTER_DIRECTIVES, CHART_DIRECTIVES, IfAuthorizeDirective,
        ViewRRFComponent, RRFGridRowComponent, RRFGridRowNewUIComponent, InterviewApprovalComponent],
    styleUrls: ['../../shared/css/RRFNewUI.component.css'],
    pipes: [RRFIDPipe, RRFPipe],
    providers: [ToastsManager]
})

export class RRFDashboardNewUI implements OnActivate {
    flag: boolean = false;
    rrfList: RRFDetails[] = [];
    errorMessage: string;
    selectedRRF: RRFDetails = new RRFDetails();
    selectedRRFDetails: RRFDetails = new RRFDetails();
    isListVisible: boolean = true;
    showAvaililityPanel: boolean = false;
    rrfStatusCount: AllRRFStatusCount[] = [];
    rrfOpenCount:AllRRFStatusCount[] = [];
    rrfClosedCount:AllRRFStatusCount[] = [];
        ...
        this._rrfDashboardService.getMyRRF(this.grdOptions)
            ...
            (results: any) => {
                if (results.RRFs !== undefined && results.RRFs.length > 0) {
                    this.grdOptions = (<any>(results)).GrdOperations;
                    this.rrfList = (<any>(results)).RRFs;
                    this.selectedRRFDetails = (<any>(results)).RRFs[0];
                } else { this.NORECORDSFOUND = true; }
            },
            error => this.errorMessage = <any>error);
    }
    ...
    getStatuswiseMyRRFCount() {
        this._rrfDashboardService.getStatuswiseMyRRFCount()
            ...
            results => {
                this.rrfStatusCount = <any>results;
                this.rrfOpenCount = <any>results[0];
                this.rrfClosedCount = <any>results[1];
                this.setValueToChart();
            },
            error => this.errorMessage = <any>error);
    }
    ...
    showRRFGrid(RRFDetails: RRFDetails) {
        this.flag=true;
        this.selectedRRFDetails = RRFDetails
    }
